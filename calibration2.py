...
Date: 10.28.2023
Author: youngjae.you
Purpose: 카메라 캘리브레이션 테스트2 
History:
   - 
INFO:
    # My calibration chessboard is 25x18 squares, 40mm squares, 24x17 verticies, resolution 1440x1080
----------------------------------------------------------------------------
"""
...
# 체스보드 설정
chessboard_size = (24, 17) # 정점 갯수
chessboard_square_size = 15 # 15mm
frame_size = (1440, 1080) # 1440x1080 해상도
...
# 3D 좌표 설정
# 체스보드의 3D 좌표를 설정한다. 체스보드의 3D 좌표는 (0,0,0), (1,0,0), (2,0,0) ... (23,16,0)으로 설정한다.
...
# 체스보드 한 칸의 크기
# 내부 파라미터 추정에서는 체스보드의 코너 간의 상대적인 위치만 중요하므로, 체스보드의 실제 물리적 크기 (예: 15x15mm)를 명시적으로 고려할 필요가 없다 
...
# 이미지 로드
images = sorted(glob.glob('images2/*.png'))
...
# 캘리브레이션 결과를 저장
pickle.dump((camera_matrix, distortion_coefficients), open( "calibration2.pkl", "wb" ))
pickle.dump(camera_matrix, open( "cameraMatrix2.pkl", "wb" ))
pickle.dump(distortion_coefficients, open( "dist2.pkl", "wb" ))
